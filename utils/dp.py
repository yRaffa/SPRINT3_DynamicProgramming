...
def inventario(estoque_inicial, demanda, margem=1.2):
    return max(10, int(estoque_inicial + margem * sum(demanda)))

def custoDiario(x, s, d, K, h, p):
    sp = s + x - d
    falta = max(-sp, 0)
    sobra = max(sp, 0)
...
def recursivoDP(demanda, K, h, p, S0):
    T = len(demanda)
    Smax = inventario(S0, demanda)
    ...
    def V(t, s):
        ...
        for x in range(0, Smax - s + 1):
            c, s_next = custoDiario(x, s, demanda[t], K, h, p)
...
# Bottom-up
def bottomUpDP(demanda, K, h, p, S0):
    T = len(demanda)
    Smax = inventario(S0, demanda)
    INF = 10**15
    ...
    for t in range(T - 1, -1, -1):
        ...
        for s in range(Smax + 1):
            ...
            best_x = 0
            for x in range(0, Smax - s + 1):
                c, s_next = custoDiario(x, s, d, K, h, p)
...
def planoReposicao(demanda, K=10.0, h=0.1, p=2.0, estoque_inicial=0):
    custo1, df1 = recursivoDP(demanda, K, h, p, estoque_inicial)
    custo2, df2 = bottomUpDP(demanda, K, h, p, estoque_inicial)
